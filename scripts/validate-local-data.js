...
async function main(){
 ...
 try{
  ...
  const scoped=await sandbox.readLocalUsage({since:ts(3),files:[file,{path:path.join(root,"old-unreadable.jsonl"),mtimeMs:base-1000,size:1}]});
  assert.equal(scoped.tokenUsage.totalTokens,150);assert.equal(scoped.scannedFiles,1);assert.equal(scoped.totalFiles,2);assert.equal(scoped.failedFiles,0,"files outside scope should not be parsed or reported as read failures");
  ...
  let dashboardScope={hasCurrentAuth:true,accountId:null,since:null}, usageReads=0;
  sandbox.dashboardScope=async()=>dashboardScope;
  sandbox.walkSessionFiles=async()=>[file];
  sandbox.resolveQuotaWithMode=async()=>integratedQuota;
  sandbox.readLocalUsage=async(options)=>{usageReads++;assert.equal(options.since,ts(3));return scoped;};
  assert.equal((await sandbox.getDashboard()).usage.available,false,"untracked login cannot be assigned all-local history");
  dashboardScope={hasCurrentAuth:true,accountId:"tracked-without-boundary",since:null};
  assert.equal((await sandbox.getDashboard()).usage.available,false,"a saved account without a switch boundary is unknown");
  assert.equal(usageReads,0,"unknown account scope must not scan token history");
  dashboardScope={hasCurrentAuth:true,accountId:"tracked",since:ts(3)};
  assert.equal((await sandbox.getDashboard()).usage.tokenUsage.totalTokens,150);

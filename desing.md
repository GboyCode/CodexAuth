...
基线：v0.1.52，2026-09-26。本文记录已经确定的视觉方向、交互规则和实现入口，供后续界面迭代使用。具体数值以对应源码为准；调整设计时同步更新本文。
...
用量页“本次本地／全部本地”切换栏与刷新按钮共用 38px 高度，刷新中不改变外部尺寸。全部账号额度总览采用五行结构：账号标题、快照状态、会话额度、周额度、重置次数。通过 CSS subgrid 对齐同一排卡片的各行，说明换行时由整排共同适配；底部重置次数增加细分隔线。当前账号、缓存账号与无记录账号均保留完整结构，无记录显示 `--` 和“暂无快照”，不显示成已耗尽。
